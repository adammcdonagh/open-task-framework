...
class TransferScriptTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            write_test_file(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_{i}", content=self.FILE_CONTENT)

        self.list = list_test_files(self.BASE_DIRECTORY, f"{self.FILE_PREFIX}_.*", delimiter=self.DELIMITER)
    ...
    def test_list_files_no_details(self):
        ...
        list = transfer.list_files(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_.*", False)
        ...
        list = transfer.list_files(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_qwhuidhqwduihqd", False)
        self.assertEqual(len(list), 0)

        # Try a more specific regex
        list = transfer.list_files(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_.*[0-9]", False)
        self.assertEqual(len(list), 10)

        # Try again but with character classes
        list = transfer.list_files(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_\\d+", False)
        self.assertEqual(len(list), 10)
    ...
    def test_move_files_basic(self):
        ...
        transfer.move_files(self.list, ",", self.MOVED_FILES_DIR, False, None, None, None, None, None)

        # Check that the files were moved
        for i in range(10):
            self.assertFalse(os.path.exists(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_{i}"))
            self.assertTrue(os.path.exists(f"{self.MOVED_FILES_DIR}/{self.FILE_PREFIX}_{i}"))
    ...
    def test_move_files_create_dest_dir_1(self):
        ...
        transfer.move_files(
            self.list, self.DELIMITER, f"{self.MOVED_FILES_DIR}/created_directory", True, None, None, None, None, None
        )
        # Check that the files were moved
        for i in range(10):
            self.assertFalse(os.path.exists(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_{i}"))
            self.assertTrue(os.path.exists(f"{self.MOVED_FILES_DIR}/created_directory/{self.FILE_PREFIX}_{i}"))
    ...
    def test_move_files_create_dest_dir_2(self):
        ...
        transfer.move_files(
            self.list, self.DELIMITER, f"{self.BASE_DIRECTORY}/created_directory", True, None, None, None, None, None
        )
        # Check that the files were moved
        for i in range(10):
            self.assertFalse(os.path.exists(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_{i}"))
            self.assertTrue(os.path.exists(f"{self.BASE_DIRECTORY}/created_directory/{self.FILE_PREFIX}_{i}"))
    ...
    def test_move_files_rename(self):
        ...
        for i in range(10):
            self.assertFalse(os.path.exists(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_{i}"))
            self.assertTrue(os.path.exists(f"{self.MOVED_FILES_DIR}/{self.FILE_PREFIX}_renamed_{i}"))
    ...
    def test_move_files_set_owner(self):
        ...
        with self.assertRaises(PermissionError):
            transfer.move_files(self.list, self.DELIMITER, self.MOVED_FILES_DIR, False, "root", None, None, None, None)

        # Check that the files were not moved
        for i in range(10):
            self.assertTrue(os.path.exists(f"{self.BASE_DIRECTORY}/{self.FILE_PREFIX}_{i}"))
            self.assertFalse(os.path.exists(f"{self.MOVED_FILES_DIR}/{self.FILE_PREFIX}_{i}"))

        # Now try setting the owner to the current user - Doesn't really make sense, but should work without throwing an exception
        transfer.move_files(
            self.list, self.DELIMITER, self.MOVED_FILES_DIR, False, os.environ.get("USER"), None, None, None, None
        )
    ...
    def test_move_files_set_group(self):
        ...
        with self.assertRaises(PermissionError):
            transfer.move_files(self.list, self.DELIMITER, self.MOVED_FILES_DIR, False, None, "root", None, None, None)
        ...
        transfer.move_files(self.list, self.DELIMITER, self.MOVED_FILES_DIR, False, None, groups[0], None, None, None)
        ...
        for i in range(10):
            file_group = os.stat(f"{self.MOVED_FILES_DIR}/{self.FILE_PREFIX}_{i}").st_gid
            # Convert group ID to name
    ...
    def tearDown(self):
        ...
        list = list_test_files(self.BASE_DIRECTORY, f"{self.FILE_PREFIX}_.*", delimiter=self.DELIMITER)
        if list:
